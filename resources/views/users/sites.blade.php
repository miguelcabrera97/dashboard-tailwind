                    ...
                    <table class="w-full leading-normal">
                        <thead>
                            <tr>
                                ...
                                <th
                                    class="px-5 py-3 text-xs font-semibold tracking-wider text-center text-gray-700 uppercase bg-gray-100 border-b-2 border-gray-200">
                                    Acciones
                                </th>
                            </tr>
                                      ...
                                      <template x-if="{{$v}}">
                                        <div>
                                          <form class="mr-5" action="{{route('publicar')}}" method="post">
                                            ...
                                            <button
                                                class="flex items-center p-3 px-4 text-white duration-300 bg-green-600 rounded-md cursor-pointer "
                                                type="submit">Publicar</button>
                                        </form>
                                        </div>
                                      </template>

                                        <a class="flex items-center p-3 px-4 mr-5 text-white duration-300 rounded-md cursor-pointer bg-violet-600 "
                                            href="/editar/{{ Auth::user()->email }}/{{ $site->siteid }}">Editar</a>

                                        <a class="flex items-center p-3 px-4 mr-5 text-white duration-300 bg-red-600 rounded-md cursor-pointer"
                                            href="/delete/{{ $site->siteid }}/{{ $site->id }}">Borrar Sitio</a>
                                        ...
                                        <template x-if="{{$v}}">
                                            <div>
                                                <form action="{{ route('despublicar') }}" method="POST">
                                                    ...
                                                    <input type="hidden" value="{{ $site->siteid }}" name="siteid">
                                                    <button
                                                        class="flex items-center p-3 px-4 text-white duration-300 bg-yellow-600 rounded-md cursor-pointer ">Despublicar</button>
                                                </form>
                                            </div>
